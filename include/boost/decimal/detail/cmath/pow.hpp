...
constexpr auto pow(T b, IntegralType p) noexcept -> std::enable_if_t<detail::is_decimal_floating_point_v<T> && std::is_integral_v<IntegralType>, T> // NOLINT(misc-no-recursion)
{
    // Calculate (b ^ p).
