...
void random_addition(T lower, T upper)
{
    ...
    for (std::size_t i {}; i < N; ++i)
    {
        ...
        const decimal128_t_t dec1 {val1};
        const decimal128_t_t dec2 {val2};

        const decimal128_t_t res = dec1 + dec2;
        const auto res_int = static_cast<T>(res);

        if (!BOOST_TEST_EQ(res_int, val1 + val2))
        {
            // LCOV_EXCL_START
            std::cerr << std::setprecision(std::numeric_limits<decimal128_t_t>::digits10)
                      << "Val 1: " << val1
                      << "\nDec 1: " << dec1
                      << "\nVal 2: " << val2
                      << "\nDec 2: " << dec2
    ...
    BOOST_TEST(isinf(std::numeric_limits<decimal128_t_t>::infinity() + decimal128_t_t{0,0}));
    BOOST_TEST(isinf(decimal128_t_t{0,0} + std::numeric_limits<decimal128_t_t>::infinity()));
    BOOST_TEST(isnan(std::numeric_limits<decimal128_t_t>::quiet_NaN() + decimal128_t_t{0,0}));
    BOOST_TEST(isnan(decimal128_t_t{0,0} + std::numeric_limits<decimal128_t_t>::quiet_NaN()));

    // Cohorts
    #if !(defined(__GNUC__) && __GNUC__ == 6)
    BOOST_TEST_EQ(decimal128_t_t(4,0) + decimal128_t_t(40, -1), decimal128_t_t(8,0));
    BOOST_TEST_EQ(decimal128_t_t(4,0) + decimal128_t_t(400, -2), decimal128_t_t(8,0));
    BOOST_TEST_EQ(decimal128_t_t(4,0) + decimal128_t_t(4000, -3), decimal128_t_t(8,0));
    BOOST_TEST_EQ(decimal128_t_t(40000000000, -10) + decimal128_t_t(4, 0), decimal128_t_t(8,0));
    #endif
}
...
void random_mixed_addition(T lower, T upper)
{
    ...
    for (std::size_t i {}; i < N; ++i)
    {
        ...
        const decimal128_t_t dec1 {val1};
        const T trunc_val_2 {static_cast<T>(decimal128_t_t(val2))};

        const decimal128_t_t res = dec1 + trunc_val_2;
        const auto res_int = static_cast<T>(res);
    ...
    BOOST_TEST(isinf(std::numeric_limits<decimal128_t_t>::infinity() + dist(rng)));
    BOOST_TEST(isinf(dist(rng) + std::numeric_limits<decimal128_t_t>::infinity()));
    BOOST_TEST(isnan(std::numeric_limits<decimal128_t_t>::quiet_NaN() + dist(rng)));
    BOOST_TEST(isnan(dist(rng) + std::numeric_limits<decimal128_t_t>::quiet_NaN()));
}
...
void random_mixed_subtraction(T lower, T upper)
{
    ...
    for (std::size_t i {}; i < N; ++i)
    {
        ...
        const T trunc_val_1 {static_cast<T>(decimal128_t_t(val1))};
        const decimal128_t_t dec2 {val2};

        const decimal128_t_t res = trunc_val_1 - dec2;
        const auto res_int = static_cast<T>(res);
...
{
    const decimal128_t_t dec1 {lhs};
    const decimal128_t_t dec2 {rhs};
    const decimal128_t_t res {dec1 - dec2};
    const auto res_int {static_cast<T>(res)};
...
void random_multiplication(T lower, T upper)
{
    ...
    BOOST_TEST(isinf(std::numeric_limits<decimal128_t_t>::infinity() * decimal128_t_t(dist(rng))));
    BOOST_TEST(isinf(decimal128_t_t(dist(rng)) * std::numeric_limits<decimal128_t_t>::infinity()));
    BOOST_TEST(isnan(std::numeric_limits<decimal128_t_t>::quiet_NaN() * decimal128_t_t(dist(rng))));
    BOOST_TEST(isnan(decimal128_t_t(dist(rng)) * std::numeric_limits<decimal128_t_t>::quiet_NaN()));
}
...
void random_mixed_multiplication(T lower, T upper)
{
    ...
    for (std::size_t i {}; i < N; ++i)
    {
        ...
        const decimal128_t_t dec1 {val1};
        const T dec2 {static_cast<T>(decimal128_t_t(val2))};
...
void random_division(T lower, T upper)
{
    ...
    for (std::size_t i {}; i < N; ++i)
    {
        ...
        const decimal128_t_t res {dec1 / dec2};
        const decimal128_t_t res_int {static_cast<double>(val1) / static_cast<double>(val2)};
    ...
    BOOST_TEST(isnan(std::numeric_limits<decimal128_t_t>::quiet_NaN() / decimal128_t_t(dist(rng))));
    BOOST_TEST(isnan(decimal128_t_t(dist(rng)) / std::numeric_limits<decimal128_t_t>::quiet_NaN()));
    BOOST_TEST(isinf(decimal128_t_t(dist(rng)) / decimal128_t_t(0)));
}
...
void random_mixed_division(T lower, T upper)
{
    ...
    for (std::size_t i {}; i < N; ++i)
    {
        ...
        const T dec1 {static_cast<T>(decimal128_t_t(val1))};
        const decimal128_t_t dec2 {val2};
        ...
        else if (!BOOST_TEST(abs(res - res_int) < decimal128_t_t(1, -1)))
        {
            // LCOV_EXCL_START
            std::cerr << "Val 1: " << val1
                      << "\nDec 1: " << dec1
    ...
    // Edge cases
    const decimal128_t_t val1 {dist(rng)};
    const decimal128_t_t zero {0, 0};
    BOOST_TEST(isnan(std::numeric_limits<decimal128_t_t>::quiet_NaN() / dist(rng)));
    BOOST_TEST(isinf(std::numeric_limits<decimal128_t_t>::infinity() / dist(rng)));
    BOOST_TEST(isnan(dist(rng) / std::numeric_limits<decimal128_t_t>::quiet_NaN()));
    BOOST_TEST_EQ(abs(dist(rng) / std::numeric_limits<decimal128_t_t>::infinity()), zero);
    BOOST_TEST(isinf(decimal128_t_t(dist(rng)) / 0));
...
void random_mixed_and()
{
    ...
    for (std::size_t i {}; i < N; ++i)
    {
        ...
        const decimal128_t_t res {dec1 & val2};
        std::uint64_t dec_int {};
        std::memcpy(&dec_int, &res, sizeof(std::uint64_t));
        const auto res_int {val1 & val2};
    ...
    for (std::size_t i {}; i < N; ++i)
    {
        ...
        const decimal128_t_t res {val1 & dec2};
        std::uint64_t dec_int {};
        std::memcpy(&dec_int, &res, sizeof(std::uint64_t));
        const auto res_int {val1 & val2};
...
{
    decimal128_t_t dec_lhs{lhs};
    decimal128_t_t dec_rhs{rhs};
