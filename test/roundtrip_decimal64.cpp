...
{
    errno = 0;
    constexpr decimal64_t_t one(1, 0);
    constexpr decimal64_t_t zero(0, 0);
    constexpr decimal64_t_t half(5, -1);
    ...
    errno = 0;
    BOOST_TEST_EQ(static_cast<T>(std::numeric_limits<decimal64_t_t>::infinity()), std::numeric_limits<T>::max()) && BOOST_TEST_EQ(errno, ERANGE);

    errno = 0;
    BOOST_TEST_EQ(static_cast<T>(-std::numeric_limits<decimal64_t_t>::infinity()), std::numeric_limits<T>::max()) && BOOST_TEST_EQ(errno, ERANGE);

    errno = 0;
    BOOST_TEST_EQ(static_cast<T>(std::numeric_limits<decimal64_t_t>::quiet_NaN()), std::numeric_limits<T>::max()) && BOOST_TEST_EQ(errno, EINVAL);

    errno = 0;
    BOOST_TEST_EQ(static_cast<T>(std::numeric_limits<decimal64_t_t>::signaling_NaN()), std::numeric_limits<T>::max()) && BOOST_TEST_EQ(errno, EINVAL);

    errno = 0;
    BOOST_TEST_EQ(static_cast<T>(half), static_cast<T>(0)) && BOOST_TEST_EQ(errno, 0);

    constexpr decimal64_t_t one_e_8(1, 8);
    BOOST_TEST_EQ(static_cast<T>(one_e_8), static_cast<T>(100'000'000)) && BOOST_TEST_EQ(errno, 0);

    constexpr decimal64_t_t one_e_8_2(1'000'000, 2);
    ...
    errno = 0;
    BOOST_TEST_EQ(static_cast<unsigned>(decimal64_t_t(dist(rng))), std::numeric_limits<unsigned>::max()) && BOOST_TEST_EQ(errno, ERANGE);

    errno = 0;
    BOOST_TEST_EQ(static_cast<unsigned long>(decimal64_t_t(dist(rng))), std::numeric_limits<unsigned long>::max()) && BOOST_TEST_EQ(errno, ERANGE);

    errno = 0;
    BOOST_TEST_EQ(static_cast<unsigned long long>(decimal64_t_t(dist(rng))), std::numeric_limits<unsigned long long>::max()) && BOOST_TEST_EQ(errno, ERANGE);
}
...
void test_roundtrip_conversion_integer(T min = T(0), T max = T(detail::max_significand))
{
    ...
    for (std::size_t i = 0; i < N; ++i)
    {
        ...
        const decimal64_t_t initial_decimal(val);
        const T return_val (initial_decimal);
        const decimal64_t_t return_decimal(return_val);
...
void test_conversion_from_float()
{
    ...
    decimal64_t_t dec_half {5, -1};
    BOOST_TEST_EQ(decimal64_t_t(half), dec_half) && BOOST_TEST_EQ(errno, 0);
    BOOST_TEST_EQ(decimal64_t_t(-half), -dec_half) && BOOST_TEST_EQ(errno, 0);

    BOOST_TEST(isnan(decimal64_t_t(std::numeric_limits<T>::quiet_NaN())));
    BOOST_TEST(isnan(decimal64_t_t(std::numeric_limits<T>::signaling_NaN())));
    BOOST_TEST(isinf(decimal64_t_t(std::numeric_limits<T>::infinity())));
    BOOST_TEST(isinf(decimal64_t_t(-std::numeric_limits<T>::infinity())));
}
...
void test_conversion_to_float()
{
    ...
    errno = 0;
    BOOST_TEST_EQ(static_cast<T>(std::numeric_limits<decimal64_t_t>::infinity()), std::numeric_limits<T>::infinity()) && BOOST_TEST_EQ(errno, 0);

    errno = 0;
    BOOST_TEST_EQ(static_cast<T>(-std::numeric_limits<decimal64_t_t>::infinity()), std::numeric_limits<T>::infinity()) && BOOST_TEST_EQ(errno, 0);

    errno = 0;
    BOOST_TEST(static_cast<T>(std::numeric_limits<decimal64_t_t>::quiet_NaN()) != std::numeric_limits<T>::quiet_NaN()) && BOOST_TEST_EQ(errno, 0);

    errno = 0;
    BOOST_TEST(static_cast<T>(std::numeric_limits<decimal64_t_t>::signaling_NaN()) != std::numeric_limits<T>::signaling_NaN()) && BOOST_TEST_EQ(errno, 0);
}
...
void test_roundtrip_integer_stream()
{
    ...
    for (std::size_t i {}; i < N; ++i)
    {
        ...
        const decimal64_t_t first_val {int_val};
        const T first_val_int {static_cast<T>(first_val)};
        std::stringstream ss;
        ss << std::setprecision(std::numeric_limits<decimal64_t_t>::digits10);
        ss << first_val;
        decimal64_t_t return_val {};
        ss >> return_val;
        ...
        if (!BOOST_TEST_EQ(first_val, return_val) || !BOOST_TEST_EQ(first_val_int, return_val_int))
        {
            ...
            std::cerr //<< std::scientific
                      //<< std::setprecision(std::numeric_limits<decimal64_t_t>::digits10)
                      << "RNG Val: " << int_val
                      << "\n    Val: " << first_val
...
void test_roundtrip_float_stream()
{
    ...
    for (std::size_t i {}; i < N; ++i)
    {
        const decimal64_t_t first_val {dist(rng)};
...
void test_roundtrip_conversion_decimal32_t_t()
{
    ...
    for (std::size_t i = 0; i < N; ++i)
    {
        const decimal64_t_t val {dist(rng)};
        const decimal32_t_t short_dec(val);
        const decimal64_t_t return_decimal {short_dec};
