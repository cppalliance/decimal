...
void random_LT(T lower, T upper)
{
    ...
    // Edge cases
    //BOOST_TEST(gcc_decimal32(dist(rng)) < std::numeric_limits<gcc_decimal32>::infinity());
    //BOOST_TEST(!(gcc_decimal32(dist(rng)) < -std::numeric_limits<gcc_decimal32>::infinity()));
    //BOOST_TEST(!(gcc_decimal32(dist(rng)) < std::numeric_limits<gcc_decimal32>::quiet_NaN()));
    //BOOST_TEST(!(std::numeric_limits<gcc_decimal32>::quiet_NaN() < std::numeric_limits<gcc_decimal32>::quiet_NaN()));
}
...
void random_mixed_LT(T lower, T upper)
{
    ...
    // Edge Cases
    //BOOST_TEST_EQ(gcc_decimal32(1) < T(1), false);
    //BOOST_TEST_EQ(gcc_decimal32(10) < T(10), false);
    //BOOST_TEST_EQ(T(1) < gcc_decimal32(1), false);
    //BOOST_TEST_EQ(T(10) < gcc_decimal32(10), false);
    //BOOST_TEST_EQ(std::numeric_limits<gcc_gcc_decimal32>::infinity() < T(1), false);
    //BOOST_TEST_EQ(-std::numeric_limits<gcc_gcc_decimal32>::infinity() < T(1), true);
    //BOOST_TEST_EQ(std::numeric_limits<gcc_gcc_decimal32>::quiet_NaN() < T(1), false);
}
...
void random_mixed_LE(T lower, T upper)
{
    ...
    BOOST_TEST(dist(rng) <= std::numeric_limits<gcc_decimal32>::infinity());
    BOOST_TEST(!(dist(rng) <= -std::numeric_limits<gcc_decimal32>::infinity()));
    BOOST_TEST(!(dist(rng) <= std::numeric_limits<gcc_decimal32>::quiet_NaN()));
}
...
{
    const gcc_decimal32 val1 {lhs};
    const T val2 {static_cast<T>(gcc_decimal32(rhs))};
...
void random_mixed_EQ(T lower, T upper)
{
    ...
    // Edge Cases
    BOOST_TEST_EQ(gcc_decimal32(1), T(1));
    BOOST_TEST_EQ(gcc_decimal32(10), T(10));
    BOOST_TEST_EQ(gcc_decimal32(100), T(100));
    BOOST_TEST_EQ(gcc_decimal32(1000), T(1000));
    BOOST_TEST_EQ(gcc_decimal32(10000), T(10000));
    BOOST_TEST_EQ(gcc_decimal32(100000), T(100000));
    BOOST_TEST_EQ(std::numeric_limits<gcc_gcc_decimal32>::quiet_NaN() == T(1), false);
    BOOST_TEST_EQ(std::numeric_limits<gcc_gcc_decimal32>::infinity() == T(1), false);
}
...
void random_SPACESHIP(T lower, T upper)
{
    ...
    BOOST_TEST((gcc_decimal32(dist(rng)) <=> std::numeric_limits<gcc_decimal32>::quiet_NaN()) == std::partial_ordering::unordered);
    BOOST_TEST((std::numeric_limits<gcc_decimal32>::quiet_NaN() <=> std::numeric_limits<gcc_decimal32>::quiet_NaN()) == std::partial_ordering::unordered);
}
...
void random_mixed_SPACESHIP(T lower, T upper)
{
    ...
    if (!BOOST_TEST((dist(rng) <=> std::numeric_limits<gcc_decimal32>::quiet_NaN()) == std::partial_ordering::unordered))
    {
        // LCOV_EXCL_START
        const auto eval {dist(rng) <=> std::numeric_limits<gcc_decimal32>::quiet_NaN()};
...
int main()
{
    ...
    constexpr auto pos_zero = boost::decimal::gcc_decimal32{0, 0, false};
    constexpr auto neg_zero = boost::decimal::gcc_decimal32{0, 0, true};
