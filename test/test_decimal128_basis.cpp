...
void test_binary_constructor()
{
    ...
    decimal128_t_t neg_one(-0b1, -6175);
    // 1 for sign
    // 00000 for combination field
    // 00000000 for exp
    // 1 for significand
    ...
    decimal128_t_t three(0b11, -6175);
    // 0 for sign
    // 00000 for combination field
    // 00000000 for exp
    // 3 for significand
    ...
    decimal128_t_t big(UINT64_MAX, -6175);
    // 0 for sign
    // 00000 for combination field
    // 00000000 for exp
...
{
    const decimal128_t_t one(0b1, 0);
    const decimal128_t_t neg_one(0b1, 0, true);
    const decimal128_t_t inf_val = boost::decimal::from_bits(boost::decimal::detail::d128_inf_mask);
    const decimal128_t_t qnan_val = boost::decimal::from_bits(boost::decimal::detail::d128_nan_mask);
    const decimal128_t_t snan_val = boost::decimal::from_bits(boost::decimal::detail::d128_snan_mask);
...
void test_float_constructor()
{
    ...
    const decimal128_t_t val(8.45150093187788174e+4929L);
    const decimal128_t_t computed_val {845150093187788174, 4912};
    BOOST_TEST_EQ(val, computed_val);

    const decimal128_t_t val2(6.277323608720053612e+4909L);
    const decimal128_t_t computed_val2 {6277323608720053612, 4891};
