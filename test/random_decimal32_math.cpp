...
void random_addition(T lower, T upper)
{
    ...
    for (std::size_t i {}; i < N; ++i)
    {
        ...
        const decimal32_t_t dec1 {val1};
        const decimal32_t_t dec2 {val2};

        const decimal32_t_t res = dec1 + dec2;
    ...
    BOOST_TEST(isinf(std::numeric_limits<decimal32_t_t>::infinity() + decimal32_t_t{0,0}));
    BOOST_TEST(isinf(decimal32_t_t{0,0} + std::numeric_limits<decimal32_t_t>::infinity()));
    BOOST_TEST(isnan(std::numeric_limits<decimal32_t_t>::quiet_NaN() + decimal32_t_t{0,0}));
    BOOST_TEST(isnan(decimal32_t_t{0,0} + std::numeric_limits<decimal32_t_t>::quiet_NaN()));
}
...
void random_mixed_addition(T lower, T upper)
{
    ...
    for (std::size_t i {}; i < N; ++i)
    {
        ...
        const decimal32_t_t dec1 {val1};
        const T trunc_val_2 {static_cast<T>(decimal32_t_t(val2))};

        const decimal32_t_t res = dec1 + trunc_val_2;
        const auto res_int = static_cast<T>(res);
    ...
    BOOST_TEST(isinf(std::numeric_limits<decimal32_t_t>::infinity() + dist(rng)));
    BOOST_TEST(isinf(dist(rng) + std::numeric_limits<decimal32_t_t>::infinity()));
    BOOST_TEST(isnan(std::numeric_limits<decimal32_t_t>::quiet_NaN() + dist(rng)));
    BOOST_TEST(isnan(dist(rng) + std::numeric_limits<decimal32_t_t>::quiet_NaN()));
}
...
void random_converted_addition(T lower, T upper)
{
    ...
    for (std::size_t i {}; i < N; ++i)
    {
        ...
        const T val1 {static_cast<T>(decimal32_t_t(dist(rng)))};
        const T val2 {static_cast<T>(decimal32_t_t(dist(rng)))};

        const decimal32_t_t dec1 {val1};
        const decimal32_t_t dec2 {val2};

        const decimal32_t_t res {dec1 + dec2};
        const decimal32_t_t comp_val {val1 + val2};
...
void random_mixed_subtraction(T lower, T upper)
{
    ...
    for (std::size_t i {}; i < N; ++i)
    {
        ...
        const T trunc_val_1 {static_cast<T>(decimal32_t_t(val1))};
        const decimal32_t_t dec2 {val2};

        const decimal32_t_t res = trunc_val_1 - dec2;
        const auto res_int = static_cast<T>(res);
...
void random_multiplication(T lower, T upper)
{
    ...
    for (std::size_t i {}; i < N; ++i)
    {
        ...
        const decimal32_t_t res {dec1 * dec2};
        const decimal32_t_t res_int {val1 * val2};
    ...
    BOOST_TEST(isinf(std::numeric_limits<decimal32_t_t>::infinity() * decimal32_t_t(dist(rng))));
    BOOST_TEST(isinf(decimal32_t_t(dist(rng)) * std::numeric_limits<decimal32_t_t>::infinity()));
    BOOST_TEST(isnan(std::numeric_limits<decimal32_t_t>::quiet_NaN() * decimal32_t_t(dist(rng))));
    BOOST_TEST(isnan(decimal32_t_t(dist(rng)) * std::numeric_limits<decimal32_t_t>::quiet_NaN()));
}
...
void random_mixed_multiplication(T lower, T upper)
{
    ...
    for (std::size_t i {}; i < N; ++i)
    {
        ...
        const decimal32_t_t dec1 {val1};
        const T dec2 {static_cast<T>(decimal32_t_t(val2))};
...
void random_division(T lower, T upper)
{
    ...
    for (std::size_t i {}; i < N; ++i)
    {
        ...
        const decimal32_t_t res {dec1 / dec2};
        const decimal32_t_t res_int {static_cast<float>(val1) / static_cast<float>(val2)};

        if (isinf(res) && isinf(res_int))
        {
        }
        else if (!BOOST_TEST(abs(res - res_int) < decimal32_t_t(1, -3)))
        {
            // LCOV_EXCL_START
            std::cerr << "Val 1: " << val1
                      << "\nDec 1: " << dec1
    ...
    BOOST_TEST(isnan(std::numeric_limits<decimal32_t_t>::quiet_NaN() / decimal32_t_t(dist(rng))));
    BOOST_TEST(isnan(decimal32_t_t(dist(rng)) / std::numeric_limits<decimal32_t_t>::quiet_NaN()));
    BOOST_TEST(isinf(decimal32_t_t(dist(rng)) / decimal32_t_t(0)));
}
...
void random_mixed_division(T lower, T upper)
{
    ...
    for (std::size_t i {}; i < N; ++i)
    {
        ...
        else if (!BOOST_TEST(abs(res - res_int) < decimal32_t_t(1, -1)))
        {
            // LCOV_EXCL_START
            std::cerr << "Val 1: " << val1
                      << "\nDec 1: " << dec1
    ...
    for (std::size_t i {}; i < N; ++i)
    {
        ...
        const T dec1 {static_cast<T>(decimal32_t_t(val1))};
        const decimal32_t_t dec2 {val2};
    ...
    // Edge cases
    const decimal32_t_t val1 {dist(rng)};
    const decimal32_t_t zero {0, 0};
    BOOST_TEST(isnan(std::numeric_limits<decimal32_t_t>::quiet_NaN() / dist(rng)));
    BOOST_TEST(isinf(std::numeric_limits<decimal32_t_t>::infinity() / dist(rng)));
    BOOST_TEST(isnan(dist(rng) / std::numeric_limits<decimal32_t_t>::quiet_NaN()));
    BOOST_TEST_EQ(abs(dist(rng) / std::numeric_limits<decimal32_t_t>::infinity()), zero);
    BOOST_TEST(isinf(decimal32_t_t(dist(rng)) / 0));
...
void random_mixed_and()
{
    ...
    for (std::size_t i {}; i < N; ++i)
    {
        ...
        const decimal32_t_t res {dec1 & val2};
        std::uint32_t dec_int {};
        std::memcpy(&dec_int, &res, sizeof(std::uint32_t));
        const auto res_int {val1 & val2};
    ...
    for (std::size_t i {}; i < N; ++i)
    {
        ...
        const decimal32_t_t res {val1 & dec2};
        std::uint32_t dec_int {};
        std::memcpy(&dec_int, &res, sizeof(std::uint32_t));
        const auto res_int {val1 & val2};
