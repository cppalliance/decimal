...
void test_generic_binary_to_decimal()
{
    ...
    BOOST_DECIMAL_IF_CONSTEXPR (std::is_same<T, double>::value)
    {
        ...
        for (const auto bit_values : test_values)
        {
            ...
            // And we convert it via direct-memory access to a decimal64_t_t.

            // In this particular example we assume that this number could
            // be the result of some calculation, initialization or any other
            // valid operations.

            const boost::decimal::decimal64_t_t dec = *reinterpret_cast<const boost::decimal::decimal64_t_t*>(&ui);

            // Now let's convert this decimal64_t_t value to built-in double.
            // ISSUE: This cast seems to result in 0.
            const auto dbl = static_cast<double>(dec);
            const boost::decimal::decimal64_t_t return_dec(dbl);

            if (!BOOST_TEST(abs(dec - return_dec) <= 1 ))
            {
                // LCOV_EXCL_START
                std::cerr << std::scientific << std::setprecision(std::numeric_limits<boost::decimal::decimal64_t_t>::digits10)
                          << "       Dec: " << dec
                          << "\n       Dbl: " << dbl
